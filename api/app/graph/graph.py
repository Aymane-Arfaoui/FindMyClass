...
def yen_k_shortest_paths(graph, start_id, end_id, K=3):
    ...
    for k in range(1, K):
        for i in range(len(paths[k-1]["path"]) - 1):
            ...
            spur_path_result = find_shortest_path(temp_graph, spur_node_id, end_id)

            if spur_path_result and spur_path_result["path"]:
                total_path_ids = root_path_ids[:-1] + spur_path_result["path"]
                total_distance = calculate_total_distance(graph, total_path_ids)
...
def calculate_total_distance(graph, path_ids):
    distance = 0.0
    for i in range(len(path_ids) - 1):
        node_a = graph.nodes[path_ids[i]]
        node_b = graph.nodes[path_ids[i+1]]
        distance += node_a.distance_to(node_b)
    return distance

# Load graph from JSON file
def load_graph_from_json(filepath, scale_factor=1):
